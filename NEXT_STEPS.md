...
### 7) 验证“1.5B 中间秩饱和”假设（新增）

最小可执行验证：

- 对 1.5B 与 7B 的 `k1/k2/k3/k5` 分别统计跨 pair 方差、IQR、分位距；检查 `k2/k3` 是否显著更“接近常量”；
- 画每个 k 的分布图（箱线图/核密度），避免只看相关系数；
- 在回归中加入 `model_size × k_bucket` 交互，验证“中间秩退化”是否为尺度特异现象。
