...
### 2.4 对“1.5B 非单调（V 形）”的批判性解释

观察层面可确认：1.5B 上相关呈 **`k1` 高、`k2/k3` 近零、`k5` 回升**；这比“`k=1` 最优”更准确。

可接受为**工作假设**的解释：

- 小模型空间更紧凑时，某些中间秩子空间相似度可能出现“**饱和**”（跨 pair 方差变小），导致相关被压平；
- `k=1` 与 `k=5` 可能分别捕捉了“主导方向差异”与“更丰富方向差异”，因此保留一定区分度。

必须保留的边界（当前尚未直接证明）：

- 仅凭相关表尚不能证明“1536 维导致几何饱和”；需要直接展示各 `k` 指标在 pair 间的分布/方差与是否接近常量；
- 也不能排除任务簇构成、样本噪声、训练幅度差异（`lora_l2_delta_b`）造成同样现象。

因此正文建议写法：**“1.5B 出现中间秩信息退化（mid-rank collapse）的现象级证据；几何饱和是当前最有解释力但待验证的假设。”**
...
### 14.3 解释边界（对“符号反转”的批判性接收）

可确认事实：在同协议同 seed 子集里，正反方向相关不仅不对称，`k3` 的 Spearman 还出现符号反转（forward 近零、reverse 明显为负）。

较可信解释（A，当前更推荐）：

- 正反方向的 `forgetting` 语义并不相同：A->B 衡量“B 对 A 的破坏”，B->A 衡量“A 对 B 的破坏”；
- 若任务簇存在系统性学习难度与更新幅度差异（例如推理任务更新更大），则可能出现“同一 overlap 指标在两个方向上作用符号不同”。

替代解释（B，仍需警惕）：

- 反向 48 点虽非极小样本，但仍可能受 pair 构成与种子结构影响；当前证据支持“值得认真对待”，尚不足以写成机制定论。

当前最稳妥表述：**遗忘不仅受任务对相似度影响，还受训练顺序与任务属性（学习速率/更新幅度）共同调制。**
